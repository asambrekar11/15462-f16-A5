...
namespace CMU462 {
  ...
  float Vertex::laplacian() const
  {
    // Implement Me! (Task 4)
    HalfedgeCIter h = halfedge();
    Vector3D ui = this->position;
    float vi = this->offset;
    float laplace = 0.0;
    do {
	  Vector3D uj = h->twin()->vertex()->position;
	  float vj = h->twin()->vertex()->offset;
	  
	  Vector3D vec_alpha = h->twin()->next()->twin()->vertex()->position;	
	  Vector3D vec_beta  = h->next()->twin()->vertex()->position;
	  
	  Vector3D alpha_i = ui - vec_alpha;
	  Vector3D alpha_j = uj - vec_alpha;
	  
	  Vector3D beta_i = ui - vec_beta;
	  Vector3D beta_j = uj - vec_beta;
	  ...
	  laplace += (0.5 * ( (1.0 / tanf(alpha) ) + (1.0 / tanf(beta) ) ) * (  vj - vi ));
