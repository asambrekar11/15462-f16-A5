...
Spectrum PathTracer::trace_ray(const Ray &r) {
	...
	if(isect.bsdf->is_delta() || r.depth == 0 || r.is_delta)
	{
...
//     DirectionalLight light(Spectrum(5.f, 5.f, 5.f), Vector3D(1.0, -1.0, 0.0));
		...
		for (int i=0; i<num_light_samples; i++) {
		  ...
		  Spectrum f = cos_theta * isect.bsdf->f(w_out, w_in);

		  // TODO:
		  // Construct a shadow ray and compute whether the intersected surface is
		  // in shadow and accumulate reflected radiance
		  Ray shadowRay(hit_p + EPS_D * dir_to_light.unit(), dir_to_light.unit());
		  Intersection shadowisect;
		  if(bvh->intersect(shadowRay, &shadowisect))
		  {
		  	Vector3D occludedDist = shadowRay.d * shadowisect.t; 
		  	if(occludedDist.norm() < dist_to_light )
			 f = Spectrum(0.f, 0.f, 0.f); 
		  }
	  
		 L_out += light_L*(f*(1.f / pdf));
	 }
	     L_out *= scale;
	}    
...
//   	return L_out;
	Vector3D wi;
	float surface_pdf;
	Spectrum f = isect.bsdf->sample_f(w_out, &wi, &surface_pdf);
	wi = o2w * wi;
	float terminateP = max(0.f, 1.f - f.illum());
	double randomBound = (double)std::rand() / (double)RAND_MAX;
	if( randomBound < terminateP || r.depth >= 4)
	{
		return L_out;
	}

	Ray newRay( hit_p + EPS_D * wi, wi, (int)r.depth + 1 );
	newRay.is_delta = isect.bsdf->is_delta();
	
    return L_out +  ( ( f * trace_ray( newRay ) ) * fabs(dot(wi, hit_n)) * (1.f /  ( surface_pdf * ( 1.f - terminateP ) ) ) ) ;
  ...
  Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
    ...
    Spectrum final_spect(0.f, 0.f, 0.f);
    Vector2D sample;
    
    for(int i = 0; i < num_samples; i++)
    {
    	sample = gridSampler->get_sample();
    	sample.x = (sample.x + (double)x ) / (double)(frameBuffer.w);
    	sample.y = (sample.y + (double)y ) / (double)(frameBuffer.h); 

    	auto sample_spect = trace_ray(camera->generate_ray(sample.x, sample.y));
    	final_spect = final_spect + sample_spect;
    }

    return final_spect * (1.f/(double)num_samples);
