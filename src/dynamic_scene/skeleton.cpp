  ...
  void Skeleton::unkeyframe(double t) {
   ...
   void Skeleton :: reachForTarget(map<Joint*, Vector3D> targets, double time )
   {
	...
	while(iter < 1000)
	{
		...
		{
			joints[i]->ikAngleGradient = Vector3D(0.0, 0.0, 0.0);
			
		}
		
		for(JointIter it = targets.begin(); it != targets.end(); it++)
		{
			it->first->calculateAngleGradient( it->first, it->second );
		}
		
		for( auto i = 0; i < joints.size(); i++)
		{
			if(joints[i]->ikAngleGradient.norm() >= EPS_D)
			{
				joints[i]->setAngle( time, joints[i]->rotation - tau * joints[i]->ikAngleGradient );
			}
			joints[i]->rotation = joints[i]->getAngle(time);
		}
