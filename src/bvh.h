...
namespace CMU462 { namespace StaticScene {
  ...
  class BVHAccel : public Aggregate {
    ...
    private:
      ...
      struct Bucket {
        Bucket() : l_bb(), r_bb(), l_count(0), r_count(), cost(INF_D) { myAxis = XAXIS; }
        Bucket(Axis incomingAxis) : l_bb(), r_bb(), l_count(0), r_count(0), cost(INF_D) { myAxis = incomingAxis; }
        double UpdateCost(double parentSurfaceArea)
        {
          cost = (double(l_count) * l_bb.surface_area() + double(r_count) * r_bb.surface_area()) / parentSurfaceArea;
          return cost;
        }
        Axis myAxis;
        BBox l_bb,r_bb;
      ...
      void Swap(Primitive* &a, Primitive* &b);
      void PrintAxis(Axis &currAxis) const;
      bool IsLessThanEquals(Axis &currAxis, Primitive* &left, Primitive* &right) const;
      size_t Partition(Axis &currAxis, size_t start, size_t end);
      void QuickSort(Axis &currAxis, size_t start, size_t end);
      bool intersectWithNodePrimitives(const Ray &ray, BVHNode *currNode) const;
      bool intersectWithNodePrimitives(const Ray &ray, BVHNode *currNode, Intersection *i) const;
      bool intersectWithNode(const Ray &ray, BVHNode *currNode) const;
      bool intersectWithNode(const Ray &ray, BVHNode *currNode, Intersection *i) const;
    protected:

      void SplitNode(BVHNode* currNode, size_t max_leaf_size);
      void DestroyNode(BVHNode* currNode);
