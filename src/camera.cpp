...
// Transforming coordinates from normal space to screen space
	  Vector3D p(x, y, 1.f);		
	  
      Matrix3x3 n2s;
      n2s(0,0) = -(double)screenW;	n2s(0,1) =  			0.f; n2s(0,2) =  (double)((double)screenW / 2.f);
      n2s(1,0) =         	  0.f; 	n2s(1,1) = -(double)screenH; n2s(1,2) =  (double)((double)screenH / 2.f);
      n2s(2,0) =			  0.f; 	n2s(2,1) = 			    0.f; n2s(2,2) =  							 1.f;
      
      p = n2s * p;
      p.z = (double)screenDist;//converting to 3D point 
      
      p = c2w * p;//tranform from camera to world space
